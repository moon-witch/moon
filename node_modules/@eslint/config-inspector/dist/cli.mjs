...
import { resolve as resolve$1, dirname, join, relative } from 'node:path';
...
async function resolveConfigPath(options) {
  ...
  const configPath = userConfigPath ? resolve$1(cwd, userConfigPath) : await findUp(configFilenames, { cwd: userBasePath || cwd });
  if (!configPath)
